...
class Apple_Valid extends curl {
    ...
    function __create_account($email) {
        $page = $this->get('https://appleid.apple.com/account#!&page=create');
        $headers = array( 
            'Accept'                => 'application/json, text/javascript, */*; q=0.01',
            'Content-Type'          => 'application/json',
            'scnt'                  => $this->getString("scnt: '", "'", $page),
            'X-Apple-Api-Key'       => $this->getString("apiKey: '", "'", $page),
            'X-Apple-ID-Session-Id' => $this->getString("sessionId: '", "'", $page),
            'X-Requested-With'      => 'XMLHttpRequest'
        );
        foreach ($headers as $key => $value) {
            $this->setHeader($key, $value);
        }
        $req = json_decode($this->post('https://appleid.apple.com/account/validation/appleid', '{"emailAddress":"' . $email . '"}'));
        if(isset($req->used)) {
            if($req->used == true) {
                $this->response['code']     = '100';
                $this->response['message']  = 'live';
                $this->response['method']   = 'create_account';
                $this->response['email']    = $email;
                return true;
            } else {
                $this->response['code']     = '200';
                $this->response['message']  = 'die';
                $this->response['method']   = 'create_account';
                $this->response['email']    = $email;
                return true;
            }
    ...
    function __forgot_password($email) {
        $headers = array( 
            'Accept'                => 'application/json, text/javascript, */*; q=0.01',
            'Content-Type'          => 'application/json',
            'X-Requested-With'      => 'XMLHttpRequest',
        ...
        $req = ($this->post('https://iforgot.apple.com/password/verify/appleid', '{"id":"' . $email . '"}'));
        $json = json_decode($req);
        if(preg_match('/<title>403 Forbidden<\/title>/', $req)) {
            $this->response['code']     = '100';
            $this->response['message']  = 'live';
            $this->response['method']   = 'forgot_password';
            $this->response['email']    = $email;
            return true;
        } elseif (isset($json->forgotPasswordFlow)) {
            $this->response['code']     = '100';
            $this->response['message']  = 'live';
            $this->response['method']   = 'forgot_password';
            $this->response['email']    = $email;
            return true;
        } elseif (isset($json->name) && !isset($json->forgotPasswordFlow)) {
            $this->response['code']     = '200';
            $this->response['message']  = 'die';
            $this->response['method']   = 'forgot_password';
            $this->response['email']    = $email;
    ...
    function check($email) {
        if(filter_var($email, FILTER_VALIDATE_EMAIL)) {
            $check = $this->__create_account($email);
            if(!$check) {
                $check = $this->__forgot_password($email);
            }
        } else {
...
$apple = new Apple_Valid();
if(isset($_GET['email']) && !empty($_GET['email'])) {
    header('Content-Type: application/json; charset=utf-8');
    echo $apple->check($_GET['email']);
    exit();
}